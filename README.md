...
BSP_SDK\
  ...
  AI\RUN.md                 ★ AI 가 혼자 끝내는 절차 (되묻지 않음 · 수선표 · 보고 형식)
  AI\wizard.ps1             넷만 묻고 바로 적용 (사람이 직접 돌릴 때)
  AI\apply.ps1              마법사가 부르는 실행부 — 점검·붙이기·빌드·검증
...
리빗 연도·애드인 프로젝트·서버 주소·설치 위치 **넷만 묻고**(전부 기본값이 채워져 있어 엔터로 넘어감)
바로 적용합니다. 묻지 않게 하려면 `-Auto`, 손으로 인자를 주려면 아래를 씁니다.
...
**OK/FAIL 표**로 끝난다. 건드리지 않고 보기만 하려면 `-Check`.
되돌리기는 언제나 `.\AI\snapshot.ps1 -Restore`.

**이미 리빗 플랫폼(애드인)을 만들어 두셨다면** :

```
https://github.com/ilovecodingma/bsp-revit-platform-sdk 받아서
AI\RUN.md 대로 알아서 다 해줘.
```

그러면 AI 가 리빗 연도·애드인 프로젝트·설치 위치를 **스스로 찾아** 붙이고, 검증하고,
FAIL 이 있으면 정해진 수선표대로 한 번 고친 뒤 **세 줄로 보고**한다.
되묻는 것은 «업데이트 서버 주소» 하나뿐이고, 그것도 모르면 오프라인으로 진행한다.
전체 그림은 [`docs\04-흐름-설계.md`](docs/04-흐름-설계.md).
...
2. 붙기만 하는지 30초 확인 : `binsp-probe.exe cycle`  (리빗도 서버도 필요 없다)
