...
**이 묶음의 성격은 제안이다.** 서버를 어떻게 세울지는 그쪽 판단에 맡긴다 —
`SPEC\API.md` 만 지키면 구현 언어·프레임워크·호스팅은 무엇이든 상관없다.
동봉한 파이썬 서버는 «이렇게 하면 돈다» 를 보여 주는 참조본일 뿐이다. → `PROPOSAL.md`
...
2. `AI\MIGRATE.md` — **지금 구조를 런처 형식으로 바꾸는 6단계.**
   각 단계의 «붙여넣을 문장» 을 AI 에 그대로 넣고 엔터, 끝날 때마다 «확인» 을 돌린다
3. `AI\TEST.md` — 확인·시험 매뉴얼. 결과를 그대로 적어 남긴다
